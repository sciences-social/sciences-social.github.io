...
It's also about to become part of the fediverse - making it possible
for folks on Mastodon servers like sciences.social to follow people on Threads and vice-versa. 

Many fediverse denizens have argued that fediverse communities should block Threads since Meta has
...
and see approach to Threads. We'll treat it like other large fediverse instances, blocking
accounts that interact with our community and violate our rules. If we see too much bad behavior
by Threads or its users we'll block it. This decision is similar to
...
some instructions at the end of this post from [infosec.exchange](https://blog.infosec.exchange/2023/12/27/threads-and-infosec-exchange/). 
And if you would like to migrate your account to an instance that pre-emptively
...
want their data to become part of Threads monetization and AI schemes.
...
[Erin Kissane](https://erinkissane.com/untangling-threads?ref=upstract.com). It's worth a read. 

### The argument for federating

There are two main arguments for federating with Threads. 

1. If we want open protocols to succeed, we are going to need corporations with
deep pockets to build the solutions that are attractive to lots of non-techy people. 
...
other newsmakers who have made Threads their current social media home.
...
indexed and searched, by Threads or anyone else. There are rules and conventions 
that the big search engines follow, but as long as it is technically possible for people to read your posts, 
without a login, someone is probably doing it.
