...
const MastodonApi = function (params_) {
  ...
  this.ERROR_ELEM_ID = params_.error_elem_id;
  ...
  this.mtErrorElem = document.getElementById(this.ERROR_ELEM_ID);
...
MastodonApi.prototype.buildTimeline = async function () {
  ...
  if (this.mtBodyContainer.innerHTML === "") {
    ...
    this.mtBodyContainer.innerHTML =
      '<div class="mt-error"><span class="mt-error-icon">📭</span><br/><strong>No recent public posts.</strong></div>';
  } else {
...
MastodonApi.prototype.getTimelineData = async function () {
  return new Promise((resolve, reject) => {
    ...
    showError = (message) => {
      if(this.mtErrorElem.innerText.trim() == "") {
          this.mtErrorElem.innerHTML =
            '<div class="mt-error"><strong>' + this.INSTANCE_URL + ' appears to be experiencing issues.</strong>'+
            'See <a href="https://mastodon.social/@sciences_social_admin">for status messages</a> and messages from our hosting provider <a href="https://mastodon.social/@masto.host">masto.host</a><br/>'+ 
